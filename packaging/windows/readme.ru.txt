...
1. CMake 2.6.x (протестированно с 2.6.2)
...
6. Qt 4.x.x (протестированно с Qt 4.4.2)
...
6. Теперь нужно сделать так, чтобы CMake видела директорию bin пакета MinGW
   ...
   2.  Можно просто запускать CMake из Qt Command Prompt, которая сама добавит
       ...
       Просто CMakeSetup.exe не запуститься, даже если вы при инсталляции
...
7. Запустить Qt Command Promt из меню Пуск.
   ...
   Когда этот шаг давершится, гляньте в файл log.txt чтобы убедиться, что
...
9. Возвращаемся в Qt Command Promt и даем там следующие команды:
   ...
   Если все прошло как надо, в текущей директирии появится файл scantailor.exe
...
5. Из Qt Command Prompt сделать следующее:
   ...
   исходное состояние, то есть заного распаковать его из .tar.gz файла.